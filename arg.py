...
# epoch = 1000
# lr = 0.05
# result_dir = 'D:\\YYH\\gas\\Result\\RNA_ATAC'
# cuda = '0'
# n_hid = 128
# n_heads = 16
...
file0='save'+'_nheads_'+str(args.n_heads)+'_nlayers_'+str(args.n_layers)
...
def debuginfoStr(info):
    ...
    print('Mem consumption (MB): '+str(mem))
...
def sub_sample(samp_nodes, sampled_depth=args.sample_depth, sampled_number=args.sample_width):
    ...
    for _type in layer_data:
        ...
        if len(layer_data[_type]) == 0:
            continue
        idxs = np.array(list(layer_data[_type].keys()), dtype=np.int32)
        # print(idxs)
...
np.savetxt(gene_dir+file0+'.txt', gene_matrix, delimiter=' ')
np.savetxt(cell_dir+file0+'.txt', cell_matrix, delimiter=' ')
